...
# File with the shuffled TFRecord filenames
SERVER_ADDRESS="0.0.0.0:50051"
FS_DRIVER="pastor://" + SERVER_ADDRESS
...
def process_record_dataset(dataset,
  ...
  if is_training:
    ...
    # Comment to use PRISMA middleware
...
def input_fn(is_training,
  ...
  #if is_training: # Comment to use PRISMA middleware
    # Shuffle the input files
    # Comment to use PRISMA middleware
    #dataset = dataset.shuffle(buffer_size=_NUM_TRAIN_FILES)
    
    # Save filenames in file
    #dataset = dataset.map(lambda x: tf.py_function(func=write_to_file, inp=[x], Tout=tf.string))
