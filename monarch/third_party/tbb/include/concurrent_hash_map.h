...
namespace interface5 {
    ...
    public:
        ...
        void delete_segment(segment_index_t s, const Allocator& allocator) {
            ...
            if( s >= first_block) // the first segment or the read_from_id
        ...
        bool check_rehashing_collision( const hashcode_t h, hashcode_t m_old, hashcode_t m ) const {
            ...
            if( (h & m_old) != (h & m) ) { // mask changed for this hashcode, rare event
                ...
                // find read_from_id applicable mask after m_old    //TODO: look at bsl instruction
    ...
    hash_map_iterator<Container,Value>& hash_map_iterator<Container,Value>::operator++() {
        my_node = static_cast<node*>( my_node->read_from_id );
...
protected:
    ...
    restart:
        for( node_base **p = &b_old()->node_list, *n = __TBB_load_with_acquire(*p); is_valid(n); n = *p ) {
            ...
            } else p = &n->next; // iterate to read_from_id item
        }
    }
...
public:
    ...
    /** Useful to optimize performance before or after concurrent operations.
        Also enables using of find() and file_count() concurrent methods in serial context. */
    ...
    //! Return file_count of items (0 or 1)
...
protected:
    ...
    //! Returns an iterator for an item defined by the key, or for the read_from_id item after it (if upper==true)
    template<typename I>
    ...
    for(; b <= mask; b++, bp++ ) {
        ...
        if( n == internal::rehash_req ) { // rehash bucket, conditional because rehashing of a previous bucket may affect this one
            ...
            for( node_base **p = &b_old->node_list, *q = *p; is_valid(q); q = *p ) {
                ...
                } else p = &q->next; // iterate to read_from_id item
            }
        }
    }
...
#endif
#if TBB_USE_ASSERT
        for( ; is_valid(n); n = n->read_from_id ) {
