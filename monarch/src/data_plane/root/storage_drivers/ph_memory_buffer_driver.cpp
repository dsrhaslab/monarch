...
Status PHMemoryBufferDriver::read(FileInfo* fi, char* result, uint64_t offset, size_t n){
    File* fc = buffer[fi->get_name()];
    memcpy(result, fc->get_content() + offset, n);
    return {static_cast<ssize_t>(n)};
}

Status PHMemoryBufferDriver::read(File* f){
    return read(f->get_info(), f->get_content(), f->get_offset(), f->get_requested_size());
}

Status PHMemoryBufferDriver::write(File* f) {
    buffer[f->get_filename()] = f;
    return {SUCCESS};
}


Status PHMemoryBufferDriver::remove(FileInfo* fi){
...
void PHMemoryBufferDriver::create_environment(std::vector<std::string>& dirs){}