...
#ifndef MONARCH_PROFILER_H
#define MONARCH_PROFILER_H
...
public:
    ...
    inline void set_n_bytes(off_t n){
        n_bytes = n;
    }

    inline off_t get_n_bytes() const{
...
protected:
    //Also used by proxy to init an empty Profiler
    void init_variables(int hierarchy_size){
    ...
    virtual inline void submit_storage_write(StatSubmission& submission, int level){
        auto& storage_write_stats = hierarchy_stats[level]->write_stats;
        //TODO this leads to deadlock. Investigate why!
        //std::lock_guard<std::mutex> l(storage_write_stats);
        update_stats(storage_write_stats, submission);
