...
void USServer::handle_request(int sock) {
    DataPlaneInstance* data_plane;
    check_for_endianness();
    //Bind info
    USClient::bind_info bi{};
    int res = ::read(sock, &bi, sizeof(bi));
    ...
    data_plane = DataPlaneInstance::get_instance(bi.rank, bi.group, bi.c_server_addr, bi.dp_server_addr);
