...
FileInfo::FileInfo(std::string f, size_t file_size, int origin_level, bool has_shareable_file_descriptors)
    ...
    if(shareable_file_descriptors){
        for(int i = 0; i <= origin_level; i++){
            descriptors.emplace_back(-1, 0, false);
        }
    }
...
void FileInfo::update_last_storage_read(){
...
bool FileInfo::client_is_reading(int level){
    return std::get<2>(descriptors[level]);
}

void FileInfo::client_started_read(int level){
    std::get<2>(descriptors[level]) = true;
}

void FileInfo::client_ended_read(int level){
    std::get<2>(descriptors[level]) = false;
}

std::tuple<int, int, bool>& FileInfo::get_descriptor_info(int level){
