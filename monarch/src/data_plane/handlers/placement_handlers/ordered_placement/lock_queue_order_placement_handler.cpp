...
LockQueueOrderPlacementHandler::LockQueueOrderPlacementHandler(HierarchicalDataPlane* data_plane, PlacementPolicy placement_policy) : OrderedPlacementHandler(data_plane, placement_policy) {}

Status LockQueueOrderPlacementHandler::inorder_place(PrefetchedFile* f){
    if(!f->is_placeholder()) {
...
Status LockQueueOrderPlacementHandler::update_queue_and_place(std::unique_lock<std::mutex> ul){
