...
SingleThreadPlacementHandler::SingleThreadPlacementHandler(HierarchicalDataPlane* data_plane, PlacementPolicy placement_policy) : OrderedPlacementHandler(data_plane, placement_policy){}

Status SingleThreadPlacementHandler::inorder_place(PrefetchedFile* f){
    if(!f->is_placeholder()) {
        int to_level = 0;
...
Status SingleThreadPlacementHandler::update_queue_and_place(){
...
Status SingleThreadPlacementHandler::place(PrefetchedFile* f){
    if(in_order(f)){
